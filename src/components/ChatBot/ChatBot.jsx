...
const ChatBot = () => {
  ...
  const sendMessage = async (message = inputValue) => {
    ...
    try {
      ...
      const data = await ChatService.sendMessage(message, conversationHistory);
          ...
          >
            {messages.length === 0 ? (
              ...
              messages.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-xs sm:max-w-sm px-3 py-2 rounded-lg text-sm ${msg.sender === 'user'
